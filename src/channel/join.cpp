...
void join(Client& client, int fd, std::vector<std::string> param)
{
	...
	Channel* channel = server.getChannelByName(channelName);
	if (!channel) {
		channel = &server.createChannel(channelName, user.id);
		channel->addOperator(user.id);
	}

	if (channel->isKicked(user.id))
		return user.handleErrors(Errors::ERR_BANNEDFROMCHAN, channelName);

	if (channel->isInviteOnly() && !channel->isInvited(user.id))
	...
	if (channel->getLimit() <= channel->getMembers().size() && !channel->isOperator(user.id))
	...
	channel->broadcast(msg, server, &user);
	user.respond(msg);
}
